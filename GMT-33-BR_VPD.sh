...
gmt grdcut TerraClimate_vpd_2020.nc -R285/328/-35/6 -Gbr_vpd.nc
gdalinfo -stats br_vpd.nc
# Minimum=0.250, Maximum=2.880, Mean=1.008, StdDev=0.383

# Make color palette
#gmt makecpt -Cbcyr -V -T0.25/2.88 > pauline.cpt
gmt makecpt -Cspectrum-rainbow-medium -V -T0.25/2.88 > pauline.cpt

ps=BR_vpd.ps
# Make background transparent image
gmt grdimage br_vpd.nc -Cpauline.cpt -R285/328/-35/6 -JM6i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
# gmt grdcontour br_vpd.nc -R -J -C1.0 -A4.0 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg285/-37.5+w15.2c/0.4c+h+o0.0/0i+ml -R -J -Cpauline.cpt \
    ...
    -Bg0.2f0.02a0.1+l"Colormap: 'spectrum-rainbow-medium' [discrete, RGB, 8 segments]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"VPD (Vapor Pressure Deficit) in Brazil (2020)" -O -K >> $ps
...
gmt psconvert BR_vpd.ps -A0.5c -E720 -Tj -Z
