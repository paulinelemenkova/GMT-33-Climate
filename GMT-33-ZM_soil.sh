...
gmt grdcut TerraClimate_soil_2018.nc -R21/34/-19/-8 -Gzm_soil.nc
gdalinfo -stats zm_soil.nc
# Minimum=1.000, Maximum=413.000, Mean=172.908, StdDev=97.815
...
gmt makecpt -Cf-30-31-32 -V -T1/413 > pauline.cpt
#gmt makecpt -Cinferno -V -T1/413 > pauline.cpt

ps=ZM_soil.ps
# Make background transparent image
gmt grdimage zm_soil.nc -Cpauline.cpt -R21/34/-19/-8 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour zm_soil.nc -R -J -C20 -A40 -Wthinner,bisque3 -O -K >> $ps
...
gmt psscale -Dg21.0/-20.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg20f2a20+l"Colormap: 'f-30-31-32' by Gnuplot from Petr Mikulík [-T1/413, continuous, RGB, 63 segments]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Soil moisture map of Zambia (2018)" -O -K >> $ps
...
gmt psconvert ZM_soil.ps -A0.5c -E720 -Tj -Z
