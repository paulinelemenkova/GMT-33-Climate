...
gmt grdcut TerraClimate_tmin_2020.nc -R297/306/-28/-19 -Gpy_tmin.nc
gdalinfo -stats py_tmin.nc
# Minimum=16.600, Maximum=25.700, Mean=22.069, StdDev=1.669
gmt makecpt -Cwysiwyg -T19/26 > pauline.cpt
...
gmt grdimage py_tmin.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
# Add raster image
gmt grdimage py_tmin.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour py_tmin.nc -R -J -C1 -A2 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg297/-28.6+w15.2c/0.4c+h+o0.0/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg1f0.1a1+l"Colormap 'wysiwyg', 20 well-separated RGB colors (-T19/26)" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Tmin (minimum temperature) in Paraguay (2020)" -O -K >> $ps
...
gmt psconvert PY_tmin.ps -A0.5c -E720 -Tj -Z
