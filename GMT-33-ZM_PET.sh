...
gmt grdcut TerraClimate_pet_2018.nc -R21/34/-19/-8 -Gzm_pet.nc
gdalinfo -stats zm_pet.nc
# Minimum=65.000, Maximum=187.000, Mean=117.700, StdDev=25.536

# Make color palette
#gmt makecpt -Cbhw2_34.cpt -V -T13/148/1 -Ic > pauline.cpt
gmt makecpt -Chsv -V -T65/187/1 > pauline.cpt
...
gmt grdimage zm_pet.nc -Cpauline.cpt -R21/34/-19/-8 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour zm_pet.nc -R -J -C10 -A10 -Wthin,blue -O -K >> $ps
...
gmt psscale -Dg21.0/-20.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg10f1a10+l"Colormap: 'hsv' colour ramp from h5utils package by S.G. Johnson, MIT [-T13/148/1, 0-63, continuous, RGB, 63 segments]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"PET (Potential Evapotranspiration) in Zambia (2018)" -O -K >> $ps
...
gmt psconvert ZM_PET.ps -A0.5c -E720 -Tj -Z
