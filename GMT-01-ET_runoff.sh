...
gmt grdcut TerraClimate_q_2018.nc -R33/48/3/15 -Get_runoff.nc
gdalinfo -stats et_runoff.nc
# Minimum=0.000, Maximum=56.000

# Make color palette
# gmt makecpt --help
gmt makecpt -Ccategorical.cpt -Mwhite -T1/56/1 > myocean.cpt
# -Ic  -V
# elevation etopo1 world elevation dem1 dem2 dem3

ps=ET_Runoff.ps
# Make background transparent image
gmt grdimage et_runoff.nc -Cmyocean.cpt -R33/48/3/15 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour et_runoff.nc -R -J -C20 -A20 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg31.5/3+w13.3c/0.15i+v+o0.0/0i+ml -R -J -Cmyocean.cpt \
    ...
    -Bg5f1a5+l"Color scale: 'drywet' dry to wet colors [R=0/200, H=0, C=HSV]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Soil runoff map of Ethiopia (2018)" -O -K >> $ps
...
gmt psconvert ET_Runoff.ps -A1.0c -E720 -Tj -Z
