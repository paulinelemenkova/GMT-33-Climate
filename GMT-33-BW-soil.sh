...
gmt grdcut TerraClimate_soil_2018.nc -R19.5/29.5/-27/-17.5 -Gbw_soil.nc
gdalinfo -stats bw_soil.nc
# Minimum=0.000, Maximum=131.000, Mean=6.079, StdDev=13.408

# Make color palette
#gmt makecpt -Csunshine_diff_12lev.cpt -V -T234/326 > pauline.cpt
#gmt makecpt -CGTS2012_ages -V -T0/131 > pauline.cpt
#gmt makecpt -CGTS2012_epochs -V -T0/131 > pauline.cpt
gmt makecpt -CGTS2012_periods -V -T0/131 > pauline.cpt
# gmt makecpt --help
#-Ic
...
gmt grdimage bw_soil.nc -Cpauline.cpt -R19.5/29.5/-27/-17.5 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour bw_soil.nc -R -J -C20 -A20 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg19.3/-27.7+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg10f1a10+l"Colormap: 'GTS2012_periods' gradient adopted from C. Heine of the University of Sydney's EarthByte group [-T0/131, discrete, RGB, 26 segments]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Soil moisture map Botswana (2018)" -O -K >> $ps
...
#
gmt pstext -R -J -N -O -K \
-F+f15p,20,violetred4+jLB >> $ps << EOF
...
gmt psconvert BW_soil.ps -A0.5c -E720 -Tj -Z
