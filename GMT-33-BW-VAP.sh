...
gmt grdcut TerraClimate_vap_2018.nc -R19.5/29.5/-27/-17.5 -Gbw_vap.nc
gdalinfo -stats bw_vap.nc
# Minimum=1.170, Maximum=2.560, Mean=1.862, StdDev=0.279

# Make color palette
gmt makecpt -Cspectrum-rainbow-medium.cpt -V -T1.17/2.56 > pauline.cpt
# gmt makecpt --help
#-Ic
...
gmt grdimage bw_vap.nc -Cpauline.cpt -R19.5/29.5/-27/-17.5 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour bw_vap.nc -R -J -C0.1 -A0.2 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg19.3/-27.7+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg0.2f0.02a0.1+l"Colormap: 'spectrum-rainbow-medium' gradient by the Open Clip Art Library [-T1.17/2.56, continuous, RGB, 6 segments]" \
    -I0.2 -By+l"VAP values" -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"VAP (Vapor Pressure) in Botswana (2018)" -O -K >> $ps
...
gmt psconvert BW_VAP.ps -A0.5c -E720 -Tj -Z
