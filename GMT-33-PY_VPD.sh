...
gmt grdcut TerraClimate_vpd_2020.nc -R297/306/-28/-19 -Gpy_vpd.nc
gdalinfo -stats py_vpd.nc
# Minimum=0.790, Maximum=2.360, Mean=1.552, StdDev=0.374
gmt makecpt -Cncar -T0.790/2.360 > pauline.cpt
...
gmt grdimage py_vpd.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
# Add raster image
gmt grdimage py_vpd.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour py_vpd.nc -R -J -C0.10 -A0.20 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg297/-28.6+w15.2c/0.4c+h+o0.0/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg0.10f0.05a0.10+l"Colormap 'ncar', continuous, RGB, 199 segments by D. H. Munro (-T0.790/2.360)" \
...
gmt psbasemap -R -J \
    ...
    -B+t"VPD (Vapor Pressure Deficit), Paraguay (2020)" -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f16p,27,red4+jLB >> $ps << EOF
299.5 -22.9 P A R A G U A Y
EOF
# geography
gmt pstext -R -J -N -O -K \
-F+jTL+f14p,22,khaki1+jLB+a-310 >> $ps << EOF
...
gmt psconvert PY_vpd.ps -A0.5c -E720 -Tj -Z
