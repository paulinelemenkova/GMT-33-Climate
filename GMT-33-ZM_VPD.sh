...
gmt grdcut TerraClimate_vpd_2018.nc -R21/34/-19/-8 -Gzm_vpd.nc
gdalinfo -stats zm_vpd.nc
# Minimum=0.080, Maximum=2.160, Mean=0.885, StdDev=0.330

# Make color palette
#gmt makecpt -Cbhw2_34.cpt -V -T13/148/1 -Ic > pauline.cpt
#gmt makecpt -CFull_saturation_spectrum_CCW -V -T0.080/2.160 > pauline.cpt
gmt makecpt -Cncar -V -T0.080/2.160 > pauline.cpt

#gmt makecpt -CPastel_Rainbow -V -T13/148/1 -Ic > pauline.cpt
# gmt makecpt --help
#-Ic
...
gmt grdimage zm_vpd.nc -Cpauline.cpt -R21/34/-19/-8 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour zm_vpd.nc -R -J -C1 -A1 -Wthinner,brown -O -K >> $ps
...
gmt psscale -Dg21.0/-20.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg0.2f0.02a0.1+l"Colormap: 'ncar' from Yorick scientific visalisation package by D.H. Munro [-T0.080/2.160, continuous, RGB, 199 segments]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"VPD (Vapor Pressure Deficit) in Zambia (2018)" -O -K >> $ps
...
# countries
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,19,black+jLB >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f11p,19,black+jLB+a-350 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f17p,19,black+jLB >> $ps << EOF
...
gmt psconvert ZM_VPD.ps -A0.5c -E720 -Tj -Z
