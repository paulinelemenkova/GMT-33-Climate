...
gmt grdcut TerraClimate_ws_2020.nc -R297/306/-28/-19 -Gpy_wind.nc
gdalinfo -stats py_wind.nc
# Minimum=1.900, Maximum=3.200, Mean=2.624, StdDev=0.235
gmt makecpt -Cjet -T1.9/3.2 > pauline.cpt
...
gmt grdimage py_wind.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
# Add raster image
gmt grdimage py_wind.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour py_wind.nc -R -J -C0.2 -A0.4+f7p,0,black+gwhite@60 -Wthick,white -O -K >> $ps
...
gmt psscale -Dg297/-28.6+w15.2c/0.4c+h+o0.0/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg0.5f0.05a0.25+l"Colormap 'jet' Dark to light blue, white, yellow and red (-T1.9/3.2 [C=RGB])" \
...
gmt psbasemap -R -J \
    ...
    -B+t"WS (Wind Speed) in Paraguay (2020)" -O -K >> $ps
...
gmt psconvert PY_wind.ps -A0.5c -E720 -Tj -Z
