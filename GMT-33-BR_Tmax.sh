...
#gmt grdcut TerraClimate_tmax_2020.nc -R285/328/-35/6 -Gbr_tmax.nc
gmt grdcut TerraClimate_tmax_1960.nc -R285/328/-35/6 -Gbr_tmax.nc
gdalinfo -stats br_tmax.nc
# Minimum=1.500, Maximum=37.600, Mean=30.428, StdDev=4.076

# Make color palette
gmt makecpt -Ctemp-c -V -T1/38 > pauline.cpt

ps=BR_tmax.ps
# Make background transparent image
gmt grdimage br_tmax.nc -Cpauline.cpt -R285/328/-35/6 -JM6i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
# gmt grdcontour br_tmax.nc -R -J -C5 -A10 -Wthinner,white -O -K >> $ps

# Add coastlines, borders, rivers
gmt pscoast -R -J -P \
    -Ia/thinner,blue -Na -N1/thick,lawngreen -W0.1p -Df -O -K >> $ps
...
gmt psscale -Dg285/-37.5+w15.2c/0.4c+h+o0.0/0i+ml -R -J -Cpauline.cpt \
    ...
    -Bg2f0.2a2+l"Colormap: 'temp-c' by UK Met Office [discrete, RGB, 34 segments]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Tmax (maximal temperature) in Brazil (average in 1960)" -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,26,blue+jLB+a-345 -Gwhite@50 >> $ps << EOF
305 -2.0 Amazonas
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,26,blue+jLB+a-45 -Gwhite@50 >> $ps << EOF
296.5 -1.2 Negro
EOF
gmt pstext -R -J -N -O -K \
