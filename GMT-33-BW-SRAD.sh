...
gmt grdcut TerraClimate_srad_2018.nc -R19.5/29.5/-27/-17.5 -Gbw_srad.nc
gdalinfo -stats bw_srad.nc
# Minimum=234.000, Maximum=326.000, Mean=295.523, StdDev=22.416

# Make color palette
#gmt makecpt -Csunshine_diff_12lev.cpt -V -T234/326 > pauline.cpt
gmt makecpt -CCat12 -V -T234/326 > pauline.cpt
#gmt makecpt -CTertiary_01b -V -T234/326 > pauline.cpt
# gmt makecpt --help
#-Ic
...
gmt grdimage bw_srad.nc -Cpauline.cpt -R19.5/29.5/-27/-17.5 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour bw_srad.nc -R -J -C20 -A20 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg19.3/-27.7+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg10f1a10+l"Colormap: 'Cat12' gradient from the NCAR Command Language for scientific data analysis and visualisation [-T234/326, discrete, RGB, 12 segments]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Downward Surface Shortwave Radiation in Botswana (2018)" -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f15p,19,purple4+jLB -Gkhaki2@60 >> $ps << EOF
22.5 -22.0 B  O  T  S  W  A  N  A
EOF
#
gmt pstext -R -J -N -O -K \
-F+f15p,20,violetred4+jLB -Grosybrown1@60 >> $ps << EOF
...
gmt psconvert BW_SRAD.ps -A0.5c -E720 -Tj -Z
