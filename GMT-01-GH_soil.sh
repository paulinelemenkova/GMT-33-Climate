...
gmt grdcut TerraClimate_soil_2018.nc -R-4/2/4/12 -Ggh_soil.nc
#gmt grdcut TerraClimate_soil_2008.nc -R33/48/3/15 -Ggh_soil.nc
#gmt grdcut GEBCO_2019.nc -R33/48/3/15 -Get_relief.nc
gdalinfo -stats gh_soil.nc
# Minimum=9.000, Maximum=276.000

# Make color palette
gmt makecpt -Cturbo.cpt -V -T9/276/1 -N -Ic > myocean.cpt
# elevation etopo1 world elevation dem1 dem2 dem3 drywet wysiwyg vik

ps=GH_Soil.ps
# Make background transparent image
gmt grdimage gh_soil.nc -Cmyocean.cpt -R-4/2/4/12 -JM5.0i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour gh_soil.nc -R -J -C20 -A20 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg-4/3.4+w12.0c/0.15i+h+o0.3/0i+ml+e -R -J -Cmyocean.cpt \
    ...
    -Bg20f5a20+l"Color scale: 'turbo' Google's Improved Rainbow Colormap for Visualization [R=9/276, C=RGB]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Soil moisture map of Ghana (2018)" -O -K >> $ps
...
0.5 10.0 Dataset: TerraClimate. Data WorldClim, CRUTS4.0. Spatial resolution: 4 km (1/24\232)
EOF

# Convert to image file using GhostScript
gmt psconvert GH_Soil.ps -A0.5c -E720 -Tj -Z
