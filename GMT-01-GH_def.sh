...
gmt grdcut TerraClimate_def_1983.nc -R-4/2/4/12 -Ggh_def.nc
gdalinfo -stats gh_def.nc
# Minimum=57.000, Maximum=220.000, Mean=161.048, StdDev=37.170

# Make color palette
gmt makecpt -Cwysiwyg.cpt -V -T57/220/5 -N > myocean.cpt
# gmt makecpt --help
# elevation etopo1 world elevation dem1 dem2 dem3

ps=GH_DEF.ps
# Make background transparent image
gmt grdimage gh_def.nc -Cmyocean.cpt -R-4/2/4/12 -JM5.0i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour gh_def.nc -R -J -C10 -A20 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg-4/3.4+w12.0c/0.15i+h+o0.3/0i+ml+e -R -J -Cmyocean.cpt \
    ...
    -Bg10f5a10+l"Color scale: 'wysiwyg' 20 well-separated RGB colors [R=57/220, C=RGB]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"DEF (Climate Water Deficit) in Ghana (1983)" -O -K >> $ps
...
0.0 10.0 Dataset: TerraClimate. Input Data WorldClim, CRUTS4.0. Spatial resolution: 4 km (1/24\232)
EOF

# Convert to image file using GhostScript
gmt psconvert GH_DEF.ps -A0.5c -E720 -Tj -Z
