...
gmt grdcut TerraClimate_tmax_2018.nc -R33/48/3/15 -Get_tmax.nc
gdalinfo -stats et_tmax.nc
# Minimum=14.400, Maximum=40.200

# Make color palette
gmt makecpt -Clapaz.cpt -V -T14/40/1 > myocean.cpt
# gmt makecpt --help
# elevation etopo1 world elevation dem1 dem2 dem3

ps=ET_tmax.ps
# Make background transparent image
gmt grdimage et_tmax.nc -Cmyocean.cpt -R33/48/3/15 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour et_tmax.nc -R -J -C5 -A20 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg31.7/3+w13.3c/0.15i+v+o0.0/0i+ml -R -J -Cmyocean.cpt \
    ...
    -Bg2f1a1+l"Colormap: 'lapaz', perceptually uniform 'rainbow' colormap by F. Crameri [14/40/1, C=RGB]" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Tmax (maximum temperature) in Ethiopia (2018)" -O -K >> $ps
...
gmt psconvert ET_tmax.ps -A0.5c -E720 -Tj -Z
