...
gmt grdcut TerraClimate_tmin_2018.nc -R33/48/3/15 -Get_tmin.nc
gdalinfo -stats et_tmin.nc
#  Minimum=-1.800, Maximum=24.200

# Make color palette
gmt makecpt -Cimola.cpt -V -T-2/24/1 > myocean.cpt
# gmt makecpt --help
# elevation etopo1 world elevation dem1 dem2 dem3

ps=ET_tmin.ps
# Make background transparent image
gmt grdimage et_tmin.nc -Cmyocean.cpt -R33/48/3/15 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour et_tmin.nc -R -J -C5 -A10 -Wthinner,white -O -K >> $ps
...
gmt psscale -Dg31.7/3+w13.3c/0.15i+v+o0.0/0i+ml -R -J -Cmyocean.cpt \
    ...
    -Bg2f1a1+l"Colormap by F.Crameri: 'imola' perceptually uniform low-lightness gradient cpt [-2/24/1, C=RGB]" \
    -I0.2 -By+l"T, Celcius" -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Tmin (minimum temperature) in Ethiopia (2018)" -O -K >> $ps
...
gmt psconvert ET_tmin.ps -A0.5c -E720 -Tj -Z
