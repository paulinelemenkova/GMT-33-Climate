...
gmt grdcut TerraClimate_aet_2020.nc -R297/306/-28/-19 -Gpy_aet.nc
gdalinfo -stats py_aet.nc
# Minimum=16.900, Maximum=178.400, Mean=114.348, StdDev=36.608
gmt makecpt -Caspectcolr -T16/179 > pauline.cpt
...
gmt grdimage py_aet.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
# Add raster image
gmt grdimage py_aet.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour py_aet.nc -R -J -C10 -A40 -Wthinner,brown -O -K >> $ps
...
gmt psscale -Dg297/-28.6+w15.2c/0.4c+h+o0.0/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg20f1a10+l"Colormap 'aspectcolr' colour ramp distributed with the GRASS GIS (-T16/179), continuous, RGB, 5 segments" \
...
gmt psbasemap -R -J \
    ...
    -B+t"AET (Actual Evapotranspiration), Paraguay (2020)" -O -K >> $ps
...
# geography
gmt pstext -R -J -N -O -K \
-F+jTL+f14p,22,BURLYWOOD4+jLB+a-310 -Gwhite@80 >> $ps << EOF
299.0 -25.0 G R A N   C H A C O
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f13p,22,BURLYWOOD4+jLB -Gwhite@90 >> $ps << EOF
...
# mountains
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,23,white+jLB+a-70 >> $ps << EOF
304.1 -22.5 Cordillera
303.9 -22.5 de Amambay
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,23,white+jLB >> $ps << EOF
304.5 -24.2 Cordillera
304.5 -24.35 de Mbaracay
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,23,white+jLB+a-320 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,23,darkbrown+jLB+a-325 >> $ps << EOF
...
gmt psconvert PY_aet.ps -A0.5c -E720 -Tj -Z
