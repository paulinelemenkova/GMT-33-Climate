...
gmt grdcut TerraClimate_PDSI_2018.nc -R21/34/-19/-8 -Gzm_pdsi.nc
gdalinfo -stats zm_pdsi.nc
# Minimum=-5.700, Maximum=16.600, Mean=7.169, StdDev=4.278

# Make color palette
#gmt makecpt -Cbhw2_34.cpt -V -T13/148/1 -Ic > pauline.cpt
gmt makecpt -Cjet -V -T-5.7/16.6 > pauline.cpt
...
gmt grdimage zm_pdsi.nc -Cpauline.cpt -R21/34/-19/-8 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour zm_pdsi.nc -R -J -C1 -A1 -Wthinner,brown -O -K >> $ps
...
gmt psscale -Dg21.0/-20.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg4f0.2a2+l"Colormap: 'jet' colour ramp from h5utils package by S.G. Johnson, MIT [-T13/148/1, 0-15, continuous, RGB, 15 segments]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Palmer Drought Severity Index (PDSI) in Zambia (2018)" -O -K >> $ps
...
gmt psconvert ZM_PDSI.ps -A0.5c -E720 -Tj -Z
