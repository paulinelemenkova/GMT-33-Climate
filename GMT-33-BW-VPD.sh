...
gmt grdcut TerraClimate_vpd_2018.nc -R19.5/29.5/-27/-17.5 -Gbw_vpd.nc
gdalinfo -stats bw_vpd.nc
#  Minimum=0.870, Maximum=3.340, Mean=2.139, StdDev=0.528

# Make color palette
gmt makecpt -Ccyan-magenta-yellow-white.cpt -V -T0.87/3.34 > pauline.cpt
# gmt makecpt --help
#-Ic
...
gmt grdimage bw_vpd.nc -Cpauline.cpt -R19.5/29.5/-27/-17.5 -JM6.5i -I+a15+ne0.75 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour bw_vpd.nc -R -J -C0.1 -A0.2 -Wthin,white -O -K >> $ps
...
gmt psscale -Dg19.3/-27.7+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg0.2f0.02a0.1+l"Colormap: 'cyan-magenta-yellow-white' gradient by the Open Clip Art Library [-T0.87/3.34, continuous, RGB, 3 segments]" \
    -I0.2 -By+l"VPD values" -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"VPD (Vapor Pressure Deficit) in Botswana (2018)" -O -K >> $ps
...
gmt psconvert BW_VPD.ps -A0.5c -E720 -Tj -Z
