...
gmt grdcut TerraClimate_PDSI_2020.nc -R297/306/-28/-19 -Gpy_pdsi.nc
gdalinfo -stats py_pdsi.nc
# Minimum=-7.680, Maximum=3.820, Mean=-2.752, StdDev=1.821
gmt makecpt -Cwysiwyg -T-7.7/3.9 > pauline.cpt
...
gmt grdimage py_pdsi.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
# Add raster image
gmt grdimage py_pdsi.nc -Cpauline.cpt -R297/306/-28/-19 -JM6.0i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour py_pdsi.nc -R -J -C1 -A1 -Wthinner,bisque3 -O -K >> $ps
...
gmt psscale -Dg297/-28.6+w15.2c/0.4c+h+o0.0/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg4f0.2a2+l"Colormap 'wysiwyg' (-T-7.7/3.9)" \
...
gmt psbasemap -R -J \
    ...
    -B+t"Palmer Drought Severity Index (PDSI), Paraguay (2020)" -O -K >> $ps
...
gmt psconvert PY_pdsi.ps -A0.5c -E720 -Tj -Z
